...
// The following sections merge the files indicated by the first comment (// #include ...) into this file
...
extern TF_Status* TF_NewStatus(void);
...
extern void TF_SetStatusFromIOError(TF_Status* s, int error_code,
                                                   const char* context);
...
// We merged nothing from c_api_macros.h, because it only contains #define statements, which are not supported by php FFI


// #include "tensorflow/c/tf_datatype.h"
// tf_datatype.h was already included

// #include "tensorflow/c/tf_status.h"
// tf_status.h was already included
...
// TF_INVALID_ARGUMENT.
extern void TF_TensorBitcastFrom(const TF_Tensor* from,
...
// #include "tensorflow/c/tf_tensor.h"
// tf_tensor.h was already included

// #include "tensorflow/core/platform/ctstring.h"
// The ctstring.h file was skipped, except for the TF_TString definition from core/platform/ctstring_internal.h
...
extern const char *TF_StringGetDataPointer(
...
extern TF_Buffer* TF_NewBuffer(void);
...
extern TF_SessionOptions* TF_NewSessionOptions(void);
...
extern TF_Graph* TF_NewGraph(void);
...
extern void TF_SetAttrTypeList(TF_OperationDescription* desc,
                                              ...
                                              int num_values);
extern void TF_SetAttrPlaceholder(TF_OperationDescription* desc,
                                                 const char* attr_name,
...
// an operation.
extern void TF_OperationAllInputs(TF_Operation* oper,
                                                 TF_Output* inputs,
                                                 int max_inputs);
...
extern void TF_ImportGraphDefOptionsSetDefaultDevice(
...
extern int TF_GraphGetFunctions(TF_Graph* g, TF_Function** funcs,
...
extern TF_WhileParams TF_NewWhile(TF_Graph* g, TF_Output* inputs,
                                                 int ninputs,
                                                 TF_Status* status);
...
// TF_NewWhile() call.
extern void TF_FinishWhile(const TF_WhileParams* params,
                                          TF_Status* status,
                                          TF_Output* outputs);
...
//
//  On failure, null.
extern TF_Function* TF_GraphToFunction(
...
//      OpDef.
extern TF_Function* TF_GraphToFunctionWithControlOutputs(
    const TF_Graph* fn_body, const char* fn_name,
    unsigned char append_hash_to_fn_name, int num_opers,
    const TF_Operation* const* opers, int ninputs, const TF_Output* inputs,
    int noutputs, const TF_Output* outputs, const char* const* output_names,
...
// representing a binary serialization of an AttrValue protocol
// buffer.
extern void TF_FunctionSetAttrValueProto(TF_Function* func,
                                                        const char* attr_name,
                                                        const void* proto,
                                                        size_t proto_len,
                                                        TF_Status* status);
...
extern void TF_FunctionGetAttrValueProto(
...
extern unsigned char TF_TryEvaluateConstant(TF_Graph* graph,
                                                           TF_Output output,
                                                           TF_Tensor** result,
                                                           TF_Status* status);
...
extern TF_Session* TF_LoadSessionFromSavedModel(
...
extern void TF_SessionPRunSetup(
    TF_Session*,
    // Input names
...
extern TF_DeprecatedSession* TF_NewDeprecatedSession(
    const TF_SessionOptions*, TF_Status* status);
extern void TF_CloseDeprecatedSession(TF_DeprecatedSession*,
                                                     TF_Status* status);
extern void TF_DeleteDeprecatedSession(TF_DeprecatedSession*,
                                                      TF_Status* status);
extern void TF_Reset(const TF_SessionOptions* opt,
...
extern void TF_ExtendGraph(TF_DeprecatedSession*,
                                          const void* proto, size_t proto_len,
                                          TF_Status*);
...
extern TF_DeviceList* TF_SessionListDevices(TF_Session* session,
                                                           TF_Status* status);
...
// Caller takes ownership of the returned TF_DeviceList* which must eventually
// be freed with a call to TF_DeleteDeviceList.
extern TF_DeviceList* TF_DeprecatedSessionListDevices(
...
extern const char* TF_DeviceListName(const TF_DeviceList* list,
                                                    int index,
                                                    TF_Status* status);
...
// If index is out of bounds, an error code will be set in the status object,
// and a null pointer will be returned.
extern const char* TF_DeviceListType(const TF_DeviceList* list,
                                                    int index,
                                                    TF_Status* status);
...
extern int64_t TF_DeviceListMemoryBytes(
...
extern uint64_t TF_DeviceListIncarnation(
    const TF_DeviceList* list, int index, TF_Status* status);
...
extern TF_Buffer* TF_GetAllOpList(void);
...
//  status - Set to OK on success and an appropriate error on failure.
extern TF_ApiDefMap* TF_NewApiDefMap(TF_Buffer* op_list_buffer,
                                                    TF_Status* status);
...
extern void TF_ApiDefMapPut(TF_ApiDefMap* api_def_map,
                                           const char* text, size_t text_len,
                                           TF_Status* status);

// Returns a serialized ApiDef protocol buffer for the TensorFlow operation
// named `name`.
extern TF_Buffer* TF_ApiDefMapGet(TF_ApiDefMap* api_def_map,
                                                 const char* name,
                                                 size_t name_len,
                                                 TF_Status* status);
...
extern TF_Server* TF_NewServer(const void* proto,
                                              size_t proto_len,
                                              TF_Status* status);
